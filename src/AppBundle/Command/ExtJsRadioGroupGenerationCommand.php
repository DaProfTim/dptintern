...
/**
 * Generator zum Erzeugen von ExtJS RadioGroups aus Doctrine Entities.
 *
 * @package AppBundle\Command
 * @copyright 2016 NPO Applications GmbH
 * @author Christopher Menke <christopher.menke@npo-applications.de>
 */
class ExtJsRadioGroupGenerationCommand extends ContainerAwareCommand
{
    /** Config-Parameter-Name. */
    const PATH_CONFIG_PARAMETER_NAME = 'extjs_radiogroup_path';

    /** Prefix, dass vor den Dateinamen gehaengt wird. */
    const FILE_PREFIX = '';

    /** Prefix fuer den ExtJS Alias (nach "widget.") */
    const EXTJS_ALIAS_PREFIX = 'radiogroup';

    /** Prefix fuer die ExtJS-Klassendefinition. */
    const EXTJS_CLASS_PREFIX = 'NpoaApp.component.radiogroup.';

    /** Standard Pfad, in dem die generierte Datei abgelegt wird. */
    const DEFAULT_PATH = 'web/ext-app/app/component/radiogroup';
    ...
    {
        $this
            ->setName('npo:extjs:generate-radiogroup')
            ->setDescription('Generiert eine ExtJS RadioGroup aus den Datenbankeintraegen der uebergebenen Entity.')
            ->addArgument('entity', InputArgument::REQUIRED, 'Entity, aus deren Datenbankeintraegen eine RadioGroup generiert werden soll, z.B. AppBundle:Besuchbarkeit')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $extJsStr = $this->getExtJsRadioGroupString($entityName, $data);
        ...
        $output->writeln("Generiere RadioGroup fuer Entity '" . $entityBundledName . "'");
        file_put_contents($filename, $extJsStr);
    }

    /**
     * Generiert und liefert die ExtJs-RadioGroup als JSON-String.
     *
     * @param string $entityName Entity-Name
     * @param array $data Daten
     * @return string JSON-String
     */
    private function getExtJsRadioGroupString($entityName, $data)
    {
        $classDef = "Ext.define('" . self::EXTJS_CLASS_PREFIX . $entityName . "', {\n";
        $classDef .= "    extend: 'Ext.form.RadioGroup',\n";
        ...
        $classDef .= "            item.name = me.name;\n";
        $classDef .= "            item.itemId = me.itemId + '_' + item.inputValue;\n";
        $classDef .= "        });\n";
        $classDef .= "        delete me.name;\n";
